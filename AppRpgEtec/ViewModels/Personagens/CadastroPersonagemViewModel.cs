...
namespace AppRpgEtec.ViewModels.Personagens
{
    ...
    public class CadastroPersonagemViewModel : BaseViewModel
    {
        ...
        public int PontosVida { get => pontosVida; set { pontosVida = value; OnPropertyChanged(nameof(PontosVida)); }}
        public int Forca { get => forca; set { forca = value; OnPropertyChanged(nameof(Forca)); }}
        public int Defesa { get => defesa; set { defesa = value; OnPropertyChanged(nameof(Defesa)); }}
        public int Inteligencia { get => inteligencia; set { inteligencia = value; OnPropertyChanged(nameof(Inteligencia)); }}
        public int Disputas { get => disputas; set { disputas = value; OnPropertyChanged(nameof(Disputas)); }}
        public int Vitorias { get => vitorias; set { vitorias = value; OnPropertyChanged(nameof(Vitorias)); }}
        public int Derrotas { get => derrotas; set { derrotas = value; OnPropertyChanged(nameof(Derrotas)); }}
       ...
       #region Commands
        ...
        public async Task SalvarPersonagem()
        {
            try
            {
                ...
                if (personagem.Id == 0)
                    await _personagemService.PostPersonagemAsync(personagem);
                else
                    await _personagemService.PutPersonagemAsync(personagem);
        ...
        public async void CarregarPersonagem()
        {
            try 
            {
                Personagem p = await _personagemService.GetPersonagemAsync(int.Parse(personagemSelecionadoId));
                ...
                TipoClasseSelecionado = ListaTiposClasse.FirstOrDefault(x => x.Id == (int)p.Classe);
            }
            catch (Exception ex)
            {
                await Application.Current.MainPage
